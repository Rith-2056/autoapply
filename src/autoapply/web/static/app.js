...
(() => {
  ...
  function renderConfig() {
    ...
    el.innerHTML = `<div class="card"><h3>Start AutoApplying</h3><div class="form-grid">
      ...
      </div><div class="status-banner" style="margin-top:12px">🇺🇸 <b>US jobs only.</b> Every job passes an eligibility gate before the browser opens: US location (state, city or "Remote - US") → target role → criteria → not a duplicate / already applied → supported platform. Non-US, "US / Canada"-style and unknown locations are skipped.</div>
      <div class="row" style="margin-top:14px"><button class="btn secondary" id="c-preview">Preview eligible jobs</button><button class="btn big" id="c-start">▶ Start AutoApply</button></div><div id="c-preview-out" style="margin-top:12px"></div></div>`;
    ...
    $('#c-preview').onclick = async () => { const c = cfg(); const p = new URLSearchParams({ target_roles: c.target_roles.join(','), locations: c.locations.join(','), categories: c.categories.join(','), excluded_companies: c.excluded_companies.join(','), excluded_locations: c.excluded_locations.join(','), retry: c.retry }); $('#c-preview-out').innerHTML = '<span class="sub">Evaluating…</span>'; const r = await api('/listings/preview?' + p); const ex = r.stats.excluded_by_check || {}; $('#c-preview-out').innerHTML = r.stats.error ? `<div class="sub">${esc(r.stats.error)}</div>` : `<div class="sub"><b>${r.stats.candidates}</b> eligible of ${r.stats.matched_filters} matching listings · skipped: ${Object.entries(ex).map(([k, v]) => `${v} ${k.replace(/_/g, ' ')}`).join(', ') || 'none'}</div><table><thead><tr><th>Company</th><th>Role</th><th>Location</th><th>Verdict</th></tr></thead><tbody>${r.listings.map((l) => `<tr><td><b>${esc(l.company)}</b></td><td>${esc(l.title)}</td><td>${esc((l.locations || []).join('; '))}</td><td><span class="pill ${l.location_verdict === 'US' ? 'OFFER' : 'SUBMITTED'}">${esc(l.location_verdict)}</span></td></tr>`).join('')}</tbody></table>${r.excluded.length ? `<details style="margin-top:10px"><summary class="sub">Skipped listings (${r.excluded.length} shown)</summary><table><tbody>${r.excluded.map((e) => `<tr><td><b>${esc(e.company)}</b></td><td>${esc(e.title)}</td><td>${esc((e.locations || []).join('; '))}</td><td class="sub">${esc(e.reason)}</td></tr>`).join('')}</tbody></table></details>` : ''}`; };
  ...
  function questionCard(q, appId, sid) {
    ...
    card.innerHTML = `<div class="q">${esc(q.label)}</div><div class="meta">${esc(q.category)} · ${q.required ? 'required' : 'optional'} · ${esc(q.reason)}</div>
      ...
      <div class="row wrap" style="margin-top:8px">${!(q.options && q.options.length) ? `<button class="mic">🎙 Speak</button><button class="ghost btn-clean">✨ Clean up</button>` : ''}<button class="ghost btn-reuse">Reuse answer</button><label style="text-transform:none;margin:0;display:flex;gap:6px;align-items:center;font-size:12px"><input type="checkbox" class="q-remember" style="width:auto" ${(q.options && q.options.length) || !['company_motivation', 'role_motivation', 'behavioral', 'long_form'].includes(q.category) ? 'checked' : ''}> Remember for this platform</label><span class="right"></span><button class="ghost btn-skip">Skip</button><button class="btn btn-confirm">Confirm answer</button></div>`;
    ...
    const cmd = (c) => api(`/autoapply/${sid}/command`, { method: 'POST', body: c }).catch((e) => toast(e.message, 'danger'));
    $('.btn-confirm', card).onclick = () => { const v = getVal(); if (!v) return toast('Nothing to confirm', 'warn'); answered[key] = { value: v }; cmd({ type: 'answer', field_id: q.field_id, value: v, source: voice.raw ? 'voice' : (q.draft && v === q.draft ? 'draft' : 'typed'), raw: voice.raw, cleaned: voice.cleaned, confidence: voice.confidence, ai_modified: voice.ai_modified, save_reusable: v.length > 60, remember_platform: $('.q-remember', card).checked }); card.style.opacity = .5; };
  ...
  pages['/settings'] = async () => {
    ...
    main.innerHTML = `<h1>Settings</h1>
      ...
      <h2>Application platforms</h2><div class="card" id="platforms-card"><div class="sub">Loading…</div></div>
  ...
  async function renderPlatforms(active = 'workday') {
    const card = $('#platforms-card'); if (!card) return;
    const d = await api('/platforms');
    const names = Object.keys(d.platforms);
    const p = d.platforms[active];
    const label = (n) => ({ workday: 'Workday', greenhouse: 'Greenhouse', lever: 'Lever', ashby: 'Ashby', smartrecruiters: 'SmartRecruiters', generic: 'Other' }[n] || n);
    card.innerHTML = `<div class="tabs" style="margin-top:0">${names.map((n) => `<button class="${n === active ? 'active' : ''}" data-p="${n}">${label(n)}</button>`).join('')}</div>
      <div class="sub">Answers here apply only to <b>${label(active)}</b> forms and win over Profile. Leave an override blank to use the profile value.${active === 'workday' ? ` Workday also needs a per-company account: ${d.workday_accounts.length ? 'configured for ' + d.workday_accounts.map(esc).join(', ') : 'none configured (set <code>WORKDAY_ACCOUNTS</code> in .env; listings without one are marked needs input)'}.` : ''}</div>
      <h3 style="margin-top:12px">Profile fields</h3><div class="form-grid">${p.fields.map((f) => `<div><label>${esc(f.label)} <span style="text-transform:none;color:var(--muted)">· profile: ${esc(f.default || '—')}</span></label><input data-path="${esc(f.path)}" value="${esc(f.override)}" placeholder="${esc(f.default || 'not set')}"></div>`).join('')}</div>
      <h3 style="margin-top:14px">Questions seen on ${label(active)} (${p.questions_seen.length})</h3>
      ${p.questions_seen.length ? `<table><thead><tr><th>Question</th><th>Last answer</th><th>Answer for this platform</th></tr></thead><tbody>${p.questions_seen.map((q) => `<tr><td>${esc(q.question_text)}<div class="sub" style="margin:0;font-size:12px">${esc(q.category)} · seen ${q.seen}×${(q.options || []).length ? ' · options: ' + q.options.map(esc).join(' / ') : ''}</div></td><td>${esc(q.last_value || '')} <span class="sub" style="margin:0">${esc(q.last_source || '')}</span></td><td><input data-q="${esc(q.question_text)}" value="${esc(p.answers[q.question_text] || '')}" placeholder="leave blank to decide per application"></td></tr>`).join('')}</tbody></table>` : '<div class="sub">No questions recorded yet. They appear here after the first application on this platform.</div>'}
      ${Object.keys(p.answers).filter((q) => !p.questions_seen.some((s) => s.question_text === q)).map((q) => `<div class="row" style="margin-top:6px"><span style="flex:1">${esc(q)}</span><input data-q="${esc(q)}" value="${esc(p.answers[q])}" style="max-width:300px"></div>`).join('')}
      <div class="row" style="margin-top:12px"><button class="btn" id="pl-save">Save ${label(active)} answers</button><input id="pl-newq" placeholder="Add a recurring question…" style="max-width:320px"><input id="pl-newa" placeholder="…and its answer" style="max-width:240px"></div>`;
    card.querySelectorAll('.tabs button').forEach((b) => b.onclick = () => renderPlatforms(b.dataset.p));
    $('#pl-save', card).onclick = async () => {
      const fields = {}; card.querySelectorAll('input[data-path]').forEach((i) => fields[i.dataset.path] = i.value);
      const answers = {}; card.querySelectorAll('input[data-q]').forEach((i) => answers[i.dataset.q] = i.value);
      const nq = $('#pl-newq', card).value.trim(); if (nq) answers[nq] = $('#pl-newa', card).value.trim();
      await api('/platforms/' + active, { method: 'PATCH', body: { fields, answers } }); toast(`${label(active)} answers saved`, 'ok'); renderPlatforms(active);
    };
  }
